//http://blog.csdn.net/lzxadsl/article/details/48859625 api参考
(function ($) {
    //'use strict';//开启严格模式
    ...
    */
    $.fn.bootstrapSelect.defaults = {
        ...
        onBeforeSelect: function (val, rec) { },
    ...
    $.fn.bootstrapSelect.methods = {
        ...
        selectWithCallBack: function (target, options, values) {
            if ($(target).data('bootstrap.select').$contentDownList) {
                setValuesWithCallBack($(target).data('bootstrap.select'), values);
            } else {
                var int = setInterval(function () {
                    if ($(target).data('bootstrap.select').$contentDownList) {
                        clearInterval(int);
        ...
        reload: function(target, options, args) {
            ...
            // 				    options['data'] = args.params;
            // 				}
            // 				if (args.multiple || !args.multiple) {
         ...
         * 加载本地数据
         */
        loadData: function (target, options, jsonList) {
            $(target).data('bootstrap.select').loadData(jsonList);
        },
        /**
         * 清空节点
         */
        ...
        disable: function (target) {
            $(target).data('bootstrap.select').disable();
        },
        /**
         * 启用控件
         */
        enable: function (target) {
            $(target).data('bootstrap.select').enable();
        }
    };
    ...
    var BootstrapSelect = function (el, options) {
        ...
        this.timeoutId_ = 0;
        this.icoUrl = '/Areas/RmcClt/images/menu_arrows.png';//下拉图标路径
        //键盘上功能键键值数组
        ...
        this.options.selItemColor = '#ccc';//被选中节点背景色，暂时使用这个
    ...
    BootstrapSelect.prototype.createSelect = function () {
        ...
        $input.wrap('<span class="errorspan"></span>');
        ...
        if ($this.options.downBorder) {
            ...
            $input.css('width', 'calc(100% - 34px)');
            $input.parent().css('border', 0);
            $('<a href="javascript:;" style="float:right;position:relative;display:inline-block;'
        			+ 'width:34px;text-align:center;height:' + (height) + 'px;border-radius:0;border:solid 1px #ccc;border-left:0;border-bottom-left-radius: 0; border-top-left-radius: 0;'
            ...
            $input.css('width', 'calc(100% - 34px)');
            $('<a href="javascript:;" style="float:right;position:relative;display:inline-block;'
        			+ 'width:32px;text-align:center;height:' + (height) + 'px;'
        			+ '">'
        			+ '<span style="margin: 14px 4.5px 13px;display: inline-block;'
        				+ 'width: 0px;height: 0px;border-top: 5px solid #555;'
        				+ 'border-right: 5px solid transparent;border-left: 5px solid transparent">'
     ...
     * 禁用控件
	 */
    BootstrapSelect.prototype.disable = function () {
        var $this = this;
        var $input = $this.$el;

        $input.attr('disabled', true);
        //a按钮点击事件
        $input.parent().find('a').unbind('click');
        //文本框点击事件,
        $input.unbind('click');
        //文本框得到焦点s
        $input.unbind('focus');
     ...
     * 启用控件
	 */
    BootstrapSelect.prototype.enable = function () {
        var $this = this;
        var $input = $this.$el;
    ...
    * 本地加载
    */
    BootstrapSelect.prototype.loadData = function (jsonList) {
        
        var $this = this;
        if (typeof jsonList == "object") {
            //创建option节点
            $this.options['data'] = jsonList;
            makeContAndShow($this, true);
            $this.options.onLoadSuccess.call($this, jsonList);
        }
        else {
            //抛出类型错误
            throw '数据类型有误，必须为数组';
    ...
    BootstrapSelect.prototype.beforeSelect = function (selItem) {

        var $this = this;
        var options = $this.options;
        var $selItem = selItem;
        options.onBeforeSelect.call(this, $selItem.data('jsonData')[$this.options.valueField], $selItem.data('jsonData'));
    };
    /**
     * 选择时触发
    ...
    BootstrapSelect.prototype.select = function (selItem, keyEvent) {
        ...
        if ($this.options.multiple) {//多选
            if ($selItem.hasClass($this.options.selItemCls)) {//选择时如果该项已被选择，则做反选动作
                ...
                $selItem.children("input:checkbox").prop("checked", false);
                unSelect = true;
            }
            else {
                ...
                $selItem.children("input:checkbox").prop("checked", true);
            }
        }
        else {
        ...
        if ($.fn.bootstrapValidator) {
            try {
                var $text = $this.$el.parent().find('input[type="text"]');
                var $form = $this.$el.parent().find('input[type="text"]').closest("form");
                var bootstrapValidator = $($form).data('bootstrapValidator');
                bootstrapValidator.updateStatus($text, bootstrapValidator.STATUS_NOT_VALIDATED);
                $form.bootstrapValidator('validateField', $text.attr("id"));
            } catch (e) {
                console.log("当前form不支持字段校验");
            }
        }
        if (unSelect) {
    ...
    BootstrapSelect.prototype.clear = function () {
        ...
        if ($this.options.emptyText != undefined && $this.options.emptyText != null) {
            ...
            itemsHtml = '<div style="width:100%;height:20px;">' + $this.options.emptyText + '</div>';
            //每行tr绑定数据，返回给回调函数
	 ...
	 * @param value 值
	 */
    function setValuesWithCallBack(bootstrapSelect, value) {
        setValues(bootstrapSelect, value,true);
    }
    /**
	 * 设置选中
	 * @param target 下拉框列表
	 ...
	 * @param value 值
	 */
    function setValues(bootstrapSelect, value,isTriggerOnSelect) {
        ...
        $this.$contentDownList.find('div').each(function (index) {
            var item = $(this);//节点
            var itemData = item.data('jsonData');
            ...
            $.each((values + '').split(options.separator), function (i, val) {
                if (val == itemVal) {
                    ...
                    if (isTriggerOnSelect) {
                        options.onSelect.call($this, itemVal, itemData);
                    }
                    return false;//break
                }
            });
    ...
    function makeContAndShow(inputDownList, isReload) {
        ...
        var itemsHtml = '';
        if (options.multiple && options.checkbox) {
            checkboxHtml = '<input type="checkbox">';
        }
        for (var i = 0; i < data_.length; i++) {
            if (options.formatter) {
                itemsHtml += '<div style="width:100%;height:34px;line-height:34px;text-indent: 12px;">' + options.formatter(data_[i]) + '</div>';
            } else {
                itemsHtml += '<div style="width:100%;height:34px;line-height:34px;text-indent: 12px;">' + (data_[i][options.textField]?checkboxHtml:"") + data_[i][options.textField] + '</div>';
            }
        }
        $this.html(itemsHtml);
